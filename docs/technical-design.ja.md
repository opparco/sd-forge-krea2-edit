...
Identity Edit LoRAが学習時に使用した系列は次のとおりである。
...
|---|---:|---|
| text | 0 | 編集指示と画像意味情報 |
| source | 1 | 参照画像のclean VAE latent |
...
参照画像は`PIL.Image`から`float32`の`B,H,W,C` tensorへ変換する。値域は
`0.0..1.0`である。
...
positiveとnegativeは同じ参照画像でencodeされる。CFG 1でnegative計算が省略される
...
| Test | 検証内容 |
|---|---|
| `test_geometry.py` | near-AR crop、large-AR fit、/16 snap、grounding縮小 |
| `test_conditioning.py` | Qwen画像注入、vision token倍率展開、engine復元、attention API adapter、二重install防止 |
| `test_forward.py` | source追加後もtarget shapeだけを返すこと |
| `test_patch.py` | 既存model wrapperとのチェーン |
...
これらはappearance pathとsemantic pathの両方へ同じ参照順序で追加する必要がある。
2参照の場合は、sceneをframe 1、subjectをframe 2として扱い、Qwenのvision blockも
同じ順序にする。
